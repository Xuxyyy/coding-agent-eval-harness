...
import {selectionVerdict} from '../../src/reports/selection-verdict.js';
import type {CaseModule, CaseQuality, CaseTier, TrialRecord} from '../../src/types/index.js';
...
test('aggregate reports primary qualities and repeat completeness', () => {
  const report = aggregate(
    [
      trial('a', 'pass', 1, 'task-effectiveness', 'execution', 'baseline'),
      trial('a', 'fail', 2, 'task-effectiveness', 'execution', 'baseline'),
      trial('b', 'error', 1, 'verification-quality', 'verification', 'challenge'),
    ],
  ...
  assert.deepEqual(report.byTier, [
    {
      tier: 'baseline', total: 2, scored: 2, errors: 0,
      passes: {count: 1, of: 2, rate: 0.5},
    },
    {
      tier: 'challenge', total: 1, scored: 0, errors: 1,
      passes: {count: 0, of: 0, rate: null},
    },
  ]);
});

test('selection verdict distinguishes met, not_met, and incomplete', () => {
  const requirement = {caseIds: ['a', 'b'], repeats: 1};
  assert.equal(selectionVerdict([trial('a', 'pass'), trial('b', 'pass')], requirement), 'met');
  assert.equal(selectionVerdict([trial('a', 'pass'), trial('b', 'fail')], requirement), 'not_met');
  assert.equal(selectionVerdict([trial('a', 'pass'), trial('b', 'error')], requirement), 'incomplete');
  assert.equal(selectionVerdict([trial('a', 'pass')], requirement), 'incomplete');
  assert.equal(
    selectionVerdict([trial('a', 'pass'), trial('a', 'pass'), trial('b', 'pass')], requirement),
    'incomplete',
  );
});
